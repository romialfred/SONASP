/*
  ...
  3. Security
    - RLS policies updated to include mining_company filtering
...
-- Only create if gold_inventory exists
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables
    ...
    RAISE NOTICE '✅ Created triggers for gold_inventory';
  END IF;
END $$;
...
BEGIN
  ...
  ELSE
    RAISE NOTICE '';
    RAISE NOTICE 'ℹ️  Table gold_inventory does not exist yet';
  END IF;
