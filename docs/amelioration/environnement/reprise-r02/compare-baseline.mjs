...
assert.equal(before.run_prefix,'QA20260907-R02');
...
for (const row of before.storage.objects) {
  ...
  else if (current.content_md5 !== row.content_md5 || current.path_md5 !== row.path_md5) changed.push({table:'storage.objects',id:row.id});
}
