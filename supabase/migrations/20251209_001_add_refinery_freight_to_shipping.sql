/*
  # Ajout des colonnes refinery_id et freight_company_id à shipping_preparations
  
  ## Problème
  Les informations de raffinerie et compagnie de fret ne s'affichent pas car:
  1. Les colonnes refinery_id et freight_company_id n'existent pas
  2. Le code enregistre les UUIDs dans shipped_to_company et shipped_to_address (colonnes TEXT)
  3. La page de détails essaie de lire depuis des colonnes inexistantes
  
  ## Solution
  1. Ajouter refinery_id UUID REFERENCES refinery_plants(id)
  2. Ajouter freight_company_id UUID REFERENCES freight_companies(id)
  3. Copier les UUIDs depuis shipped_to_company/shipped_to_address vers les nouvelles colonnes
  4. Garder les anciennes colonnes pour compatibilité
  
  ## Tables Impactées
  - shipping_preparations (ajout 2 colonnes + indexes)
*/
...
-- (shipped_to_address contenait des UUIDs au lieu d'adresses)
DO $$
DECLARE
  v_migrated_refinery INT := 0;
  v_migrated_freight INT := 0;
BEGIN
  -- Tenter de copier les UUIDs valides depuis shipped_to_address vers refinery_id
  ...
  -- Tenter de copier les UUIDs valides depuis shipped_to_company vers freight_company_id
  UPDATE shipping_preparations sp
