...
| A217 | La page de connexion peignait une capture d'écran de 1,4 Mo sur toute la fenêtre en `object-fit: fill`, puis posait les champs par-dessus à des décalages en pixels (`top: 58px`, `left: 42px`) : la mise en page n'était juste qu'aux largeurs pour lesquelles ces décalages avaient été réglés | Connexion | Écran entièrement dessiné en CSS ; décor en trois couches, diagonale en `clip-path` ; 3,5 Mo d'images ramenés à 35 Ko |
| A218 | Le titre du panneau d'accueil ressortait en bleu nuit sur vert foncé — contraste 1,19 : 1, illisible. Une règle globale sur `h1` écrasait la couleur héritée | Connexion | Couleur posée explicitement ; contraste porté à 14,95 : 1, et l'ensemble des textes de l'écran vérifié au-dessus du seuil AA (minimum 4,76 : 1) |
