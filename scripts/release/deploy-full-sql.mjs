...
if (!['rehearse', 'apply'].includes(mode) || !cli || !folder) throw new Error('mode CLI audit-folder required');
...
function query(sql,label) {
  ...
  try {
    const raw = execFileSync(cli,['db','query','--linked','--file',file,'-o','json'],{encoding:'utf8',timeout:120000,maxBuffer:20*1024*1024,stdio:['ignore','pipe','pipe']});
...
const excluded = ['inventory_transactions','gold_inventory','snp_data_quality_issues','snp_sod_legacy_review','snp_role_module_ceilings','snp_role_capabilities','snp_rpc_execution_allowlist','snp_account_deletion_dependency_registry'];
...
const checks = `DO $c$ BEGIN
 IF EXISTS(SELECT * FROM release_before EXCEPT SELECT a.* FROM pg_temp.release_fingerprints() a JOIN release_before b USING(relation)) THEN RAISE EXCEPTION 'Business/account data changed'; END IF;
...
const sql = ["BEGIN ISOLATION LEVEL REPEATABLE READ; SET LOCAL lock_timeout='3s'; SET LOCAL statement_timeout='45s';",
 "SELECT pg_advisory_xact_lock(hashtextextended('sonasp-full-release-20260830',0));",guard,fingerprint,
 ...
 `SELECT jsonb_build_object('mode',${quote(mode)},'protected_tables',(SELECT count(*) FROM release_before),'physical_assets_unchanged',true,'owner_admin_unchanged',true) validation;`,
