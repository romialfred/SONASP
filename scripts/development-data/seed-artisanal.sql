DO $artisanal$
DECLARE
  tag constant text:='HIST-2024-2026'; mon record; m integer; n integer; k integer; artisan_no integer;
...
BEGIN
  ...
  FOR n IN 1..3 LOOP
    PERFORM pg_temp.seed_put('artisanal_sites','artisan-site',n,jsonb_build_object('code','SA-'||lpad(n::text,3,'0'),
      'name',(ARRAY['Site artisanal de Saaba','Site artisanal de Ziniaré','Site semi-mécanisé de Manga'])[n],
      'status','active','region',(ARRAY['Centre','Plateau-Central','Centre-Sud'])[n],
      'province',(ARRAY['Kadiogo','Oubritenga','Zoundwéogo'])[n],
      'locality',(ARRAY['Saaba','Ziniaré','Manga'])[n],
  ...
  FOR n IN 1..24 LOOP
    PERFORM pg_temp.seed_put('snp_artisans_miniers','artisan',n,jsonb_build_object('type_personne','physique',
      'type_artisan','exploitant','nom','ARTISAN '||lpad(n::text,2,'0'),'prenoms','Historique',
      'telephone','7000'||lpad(n::text,4,'0'),'email','artisan-'||n||'@example.invalid','nationalite','Burkinabè',
      'numero_carte','CAM-'||to_char(2024000+n,'FM0000000'),'type_piece_identite','Autre','numero_piece_identite','PID-'||to_char(2024000+n,'FM0000000'),
    ...
    PERFORM pg_temp.seed_put('snp_artisan_moyens_paiement','artisan-method',n,jsonb_build_object(
      'artisan_id',pg_temp.seed_id('artisan',n),'type','virement_bancaire','titulaire','ARTISAN '||lpad(n::text,2,'0'),
      'banque','Banque du scénario historique','numero_compte','BFHIST'||lpad(n::text,10,'0'),'libelle','Coordonnées synthétiques non vérifiées',
  ...
  FOR mon IN SELECT * FROM seed_months ORDER BY month_no LOOP
    ...
    FOR n IN 1..6 LOOP
      ...
      ref:='VA-'||to_char(d,'YYYY')||'-'||lpad((m*10+n)::text,5,'0');
      ...
      IF n<>4 THEN
        ...
        PERFORM pg_temp.seed_put('snp_artisan_factures_definitives','artisan-invoice',k,jsonb_build_object(
          ...
          'statut','emise','certification_dgi_status','pending','notes',tag||' - Facture synthétique non certifiée DGI.',
    ...
    PERFORM pg_temp.seed_actor(1);
    IF m%3=0 THEN
      PERFORM pg_temp.seed_put('snp_requisitions','requisition',m,jsonb_build_object('reference','REQ-'||to_char(d,'YYYY')||'-'||lpad(m::text,4,'0'),
        'objet','Réquisition trimestrielle de collecte aurifère','mining_company_id',pg_temp.seed_id('mine',3),
  ...
  FOR y IN 2024..2026 LOOP
    FOR n IN 1..3 LOOP
      PERFORM pg_temp.seed_put('snp_contrats','contract',(y-2024)*3+n,jsonb_build_object(
        'numero_contrat','CTR-'||y||'-'||lpad(n::text,3,'0'),'intitule','Contrat annuel d''approvisionnement aurifère '||y,
    ...
    END LOOP;
  END LOOP;
  INSERT INTO seed_results VALUES('artisanal',192,jsonb_build_object('artisans',24,'sites',3,'invoices_pending_dgi',160,
    'payment_methods_unverified',24,'requisitions_draft',11,'contracts_draft',9,
