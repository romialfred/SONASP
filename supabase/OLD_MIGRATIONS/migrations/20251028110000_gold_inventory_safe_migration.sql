/*
  # Gold Inventory Management and Batch Workflow Enhancement (SAFE MIGRATION)
  ...
  ## Approach
  - Uses a temporary table to safely migrate data
  - Does NOT disable system triggers (avoiding permission issues)
  - Creates new constraint after data is migrated
  - Fully transactional and safe
...
-- Ensure sales table exists (needed for foreign keys)
DO $$
BEGIN
...
-- Create a temporary table to hold migrated data
CREATE TEMP TABLE batches_temp AS
SELECT * FROM batches;

-- Update statuses in temporary table
...
-- Handle any remaining unknown statuses
UPDATE batches_temp SET status = 'pending_factory_approval'
WHERE status NOT IN (
...
-- ============================================================================
-- STEP 4: Update the actual table with migrated data
-- ============================================================================

-- Update batches table from temp table
UPDATE batches b
SET status = t.status
FROM batches_temp t
WHERE b.id = t.id;
...
-- ============================================================================
-- STEP 6: Create batch_approvals table
-- ============================================================================
...
-- ============================================================================
-- STEP 7: Create system_parameters table
-- ============================================================================
...
-- ============================================================================
-- STEP 8: Create gold_inventory table
-- ============================================================================
...
-- ============================================================================
-- STEP 9: Create inventory_transactions table
-- ============================================================================
