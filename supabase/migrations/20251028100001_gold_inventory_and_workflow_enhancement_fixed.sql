/*
  # Gold Inventory Management and Batch Workflow Enhancement (FIXED)
  ...
  ## Fixes
  - Properly handles existing batch statuses before constraint modification
  - Maps old statuses to new statuses
  - Checks for missing tables and columns
  - Handles foreign key references safely
...
-- Step 1: Check and create missing tables/columns if needed

-- Check if refining_records table exists, if not create a placeholder
DO $$
BEGIN
...
-- Step 2: Map existing batch statuses to new statuses before changing constraint
-- This prevents the constraint violation error

UPDATE batches SET status = 'pending_factory_approval' WHERE status = 'draft' OR status = 'pending';
UPDATE batches SET status = 'approved_for_transport' WHERE status = 'approved';
UPDATE batches SET status = 'waiting_airport_receipt' WHERE status = 'shipped' OR status = 'in_transit';
UPDATE batches SET status = 'received_at_airport' WHERE status = 'at_airport' OR status = 'received_airport';
UPDATE batches SET status = 'validated_for_refinery' WHERE status = 'validated_airport';
UPDATE batches SET status = 'waiting_refinery_receipt' WHERE status = 'in_transit_refinery';
UPDATE batches SET status = 'received_at_refinery' WHERE status = 'at_refinery' OR status = 'received_refinery';
UPDATE batches SET status = 'validated_for_processing' WHERE status = 'validated_refinery' OR status = 'ready_for_processing';
UPDATE batches SET status = 'processing' WHERE status = 'processing' OR status = 'refining';
UPDATE batches SET status = 'in_inventory' WHERE status = 'refined' OR status = 'completed';
UPDATE batches SET status = 'ready_for_sale' WHERE status = 'available' OR status = 'ready_to_sell';
UPDATE batches SET status = 'allocated_to_sale' WHERE status = 'allocated' OR status = 'reserved';
UPDATE batches SET status = 'sold' WHERE status = 'sold' OR status = 'delivered';
UPDATE batches SET status = 'cancelled' WHERE status = 'cancelled' OR status = 'rejected';

-- Step 3: Now safely drop and recreate the constraint
...
-- Step 4: Create batch_approvals table
...
-- Step 5: Create system_parameters table
...
-- Step 6: Create gold_inventory table
