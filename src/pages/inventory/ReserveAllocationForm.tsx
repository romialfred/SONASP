    ...
    <NationalDashboardLayout>
      <main className="sn-page reserve-page reserve-form-page">
        <PageHeader
          ...
          breadcrumb={[{ label: 'Réserve nationale', to: '/inventory' }, { label: 'Affectations à la réserve', to: '/national-reserve/allocations' }, { label: allocation ? allocation.reference : 'Nouvelle affectation' }]}
          actions={<>
        ...
        <div className="reserve-form-layout">
          ...
          <aside className="reserve-summary" aria-label="Résumé de l’affectation"><header><h3>Résumé de l’affectation</h3><ChevronUp /></header><section><h4>Statut</h4><div className="reserve-summary__status"><Badge tone={allocation ? RESERVE_STATUS_TONES[allocation.status] : 'warning'}>{allocation ? RESERVE_STATUS_LABELS[allocation.status] : 'Brouillon'}</Badge><span>Étape actuelle <b>{step} / 4</b></span><progress max="4" value={step} /></div></section><SummarySection title="Détails de la sélection" rows={[["Lots sélectionnés", String(summary.lots)], ["Nombre total de lingots", String(summary.ingots)], ["Poids brut total", formatGrams(summary.gross)], ["Pureté moyenne", formatPercent(summary.purity)], ["Poids d’or fin total", formatGrams(summary.fine)], ["Valeur indicative (FCFA)", allocation ? formatFcfa(allocation.indicative_value_fcfa) : 'Calculée à l’enregistrement'], ["Valeur indicative (USD)", allocation ? formatUsd(allocation.indicative_value_usd) : '—'], ["Valeur indicative (EUR)", allocation ? formatEur(allocation.indicative_value_eur) : '—'], ["Source des cours", allocation?.valuation_source || 'LBMA / BCEAO'], ["Cours horodaté", allocation ? formatDateTime(allocation.valuation_at) : 'À l’enregistrement']]} /><SummarySection title="Destination prévue" rows={[["Dépositaire", depository?.short_name || depository?.name || 'À définir'], ["Type de dépôt", formatDepositType(payload.deposit_type)], ["Localisation", depository?.address || depository?.administrative_region || 'À définir'], ["Référence dépôt", payload.planned_deposit_reference || 'À renseigner'], ["Date prévue de transfert", formatDate(payload.planned_transfer_date)]]} /><SummarySection title="Créé par" rows={[[user?.full_name || 'Utilisateur SONASP', allocation ? formatDate(allocation.created_at) : 'À l’enregistrement']]} /></aside>
        </div>
      </main>
    </NationalDashboardLayout>
  );
