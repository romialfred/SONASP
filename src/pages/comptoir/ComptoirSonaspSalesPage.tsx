...
import { ArrowRight, BadgeCheck, Plus, RefreshCw, Store, X } from 'lucide-react';
import { NationalDashboardLayout } from '@/components/layout/NationalDashboardLayout';
import { useComptoirWorkspace } from '@/hooks/useComptoirWorkspace';
import { comptoirPortalService, type ComptoirSonaspSale } from '@/services/comptoirPortalService';
...
export default function ComptoirSonaspSalesPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setSales(saleRows);
      setStockGrams(stockRows.reduce(
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (quantityGrams > stockGrams) {
      setError(`Le stock disponible est de ${decimal.format(stockGrams)} g.`);
      return;
    }
    setSaving(true);
    try {
    ...
    <NationalDashboardLayout>
      <main className="comptoir-page" aria-busy={loading}>
        <header className="comptoir-page__heading">
          ...
          <div className="comptoir-heading-actions"><button className="comptoir-button is-secondary" type="button" onClick={() => void load()} disabled={loading}><RefreshCw className={loading ? 'is-spinning' : ''} aria-hidden="true" />Actualiser</button><button className="comptoir-button is-primary" type="button" onClick={() => { setError(null); setDialogOpen(true); }} disabled={loading || stockGrams <= 0}><Plus aria-hidden="true" />Nouvelle cession</button></div>
        </header>
        ...
        <div className="comptoir-buyer-lock"><BadgeCheck aria-hidden="true" /><div><strong>Acheteur verrouillé : SONASP</strong><p>Stock disponible : {decimal.format(stockGrams)} g · aucune destination internationale ou tierce.</p></div></div>
