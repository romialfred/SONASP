/*
  ============================================
  CREATE SECONDARY DISTRIBUTION SYSTEM
  ============================================

  This creates a new table to handle secondary distribution:
  - When a customer (like MMME or HBR) acts as an intermediary
  - They redistribute to multiple end customers with specific percentages

  Examples:
  - MMME distributes: 93% Auranet, 5% Aurion, 2% CIG
  - HBR distributes: 100% Auramet
...
CREATE TABLE IF NOT EXISTS secondary_distributions (
  ...
  -- Constraint: An intermediary cannot distribute to themselves
...
BEGIN
  ...
  IF total_percentage > 100 THEN
    RAISE EXCEPTION 'Total distribution percentage for intermediary cannot exceed 100%%. Current total would be: %%', total_percentage;
  END IF;
...
CREATE TRIGGER check_distribution_percentages
  ...
  FOR EACH ROW
  EXECUTE FUNCTION validate_distribution_percentages();
...
BEGIN
  ...
  DELETE FROM secondary_distributions
  WHERE intermediary_customer_id IN (v_mmme_id, v_hbr_id);
  ...
  IF v_mmme_id IS NOT NULL THEN
    ...
    IF v_auranet_id IS NOT NULL THEN
      INSERT INTO secondary_distributions (
        ...
        v_admin_id,
        v_admin_id,
        'MMME distributes 93% to Auranet'
      );
  ...
  IF v_hbr_id IS NOT NULL AND v_auramet_id IS NOT NULL THEN
    ...
    INSERT INTO secondary_distributions (
      ...
      v_admin_id,
      v_admin_id,
      'HBR distributes 100% to Auramet'
    );
    RAISE NOTICE 'Created: HBR → Auramet (100%)';
  END IF;

  RAISE NOTICE '=== Secondary Distributions Setup Complete ===';
END $$;
