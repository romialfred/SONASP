/*
  ...
  PROBLEM:
    ...
    • Table data fails to load
    • Statistics load successfully (Total: 1, In Review: 1)
...
BEGIN
  ...
  ELSE
    RAISE NOTICE '❌ title column MISSING (will add now)';
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE '✅ Updated existing records with meaningful titles';

    -- Remove default (new records must provide title)
    ALTER TABLE license_requests
    ALTER COLUMN title DROP DEFAULT;

    RAISE NOTICE '✅ Removed default constraint';
    RAISE NOTICE '========================================';
...
BEGIN
  ...
  IF col_exists THEN
    RAISE NOTICE '✅ title column exists: YES';
  ELSE
    RAISE NOTICE '❌ title column exists: NO';
  END IF;

  -- Count records
  SELECT COUNT(*) INTO record_count FROM license_requests;
  RAISE NOTICE '📊 Total license requests: %', record_count;
