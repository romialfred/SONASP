...
const sqlFile=path.join(folder,'cleanup-candidate.sql');
const sha=file=>crypto.createHash('sha256').update(fs.readFileSync(file)).digest('hex');
const expected='3da1169b8d0497d277c12aed2715bbca099c096692d0ac520a55fd2161238650';
...
const receiptFile=path.join(folder,'cleanup-execution.json');
const resultFile=path.join(folder,'cleanup-result.json');
...
const receipt={startedAt:new Date().toISOString(),projectRef:'yyverzuhkdonjjuficor',candidateSha256:expected,
 ...
 command:'supabase 2.115.0 db query --linked --file cleanup-candidate.sql -o json',
